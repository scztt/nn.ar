...
)
{
    ...
    if (out_n_vec != n_vec)
    {
        std::cout << "model output size is not consistent, expected " << n_vec << " samples, got "
                  << out_n_vec << "!\n";
        return;
    }
...
{
    std::unique_lock<std::mutex> model_lock(m_model_mutex);
    for (auto const& m : m_model.get_methods())
    {
...
{
    for (auto const& a : get_settable_attributes())
    {
        if (a == attribute)
            return true;
    }
...
std::vector<std::string> Backend::get_available_attributes()
{
    ...
    std::unique_lock<std::mutex> model_lock(m_model_mutex);
    for (auto const& attribute : m_model.named_attributes())
...
std::vector<std::string> Backend::get_settable_attributes()
{
    ...
    {
        std::unique_lock<std::mutex> model_lock(m_model_mutex);
        for (auto const& a : m_model.named_attributes())
        {
            try
            {
...
std::vector<int> Backend::get_method_params(std::string method)
{
    ...
    if (std::find(m_available_methods.begin(), m_available_methods.end(), method)
...
{
    int higher_ratio = 1;
    for (auto const& method : m_available_methods)
    {
        auto params = get_method_params(method);
        if (!params.size())
